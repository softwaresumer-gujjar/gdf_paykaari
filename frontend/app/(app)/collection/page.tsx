...
import { collectionApi, farmersApi } from '@/lib/api';
import { useLang } from '@/lib/language-context';
import { useSubscription } from '@/lib/subscription-context';

interface Farmer { id: string; name: string; contractedMaunds: number }
interface Entry {
...
interface BulkRow {
  farmerId: string; name: string; contractedMaunds: number;
  morning: string; evening: string; fatPercent: string;
  waterAdded: boolean; qualityPassed: boolean;
}
type SortKey = 'farmer' | 'session' | 'collectedMaunds';
...
export default function CollectionPage() {
  ...
  const [sortKey, setSortKey] = useState<SortKey>('farmer');
  const [sortDir, setSortDir] = useState<SortDir>('asc');

  const load = async () => {
    const [e, s] = await Promise.all([collectionApi.list(date, 1, 200), collectionApi.summary(date)]);
    setEntries(e.data?.items ?? e.data ?? []);
    setSummary(s.data);
  };

  useEffect(() => {
    farmersApi.list().then((r) => {
      const active = (r.data as (Farmer & { isActive?: boolean })[]).filter((f) => f.isActive !== false);
      setRows(active.map((f) => ({
        farmerId: f.id, name: f.name,
        contractedMaunds: Number(f.contractedMaunds),
        morning: '', evening: '', fatPercent: '',
        waterAdded: false, qualityPassed: true,
      })));
    });
  }, []);
  ...
  const sortedEntries = useMemo(() => [...entries].sort((a, b) => {
    let cmp = 0;
    if (sortKey === 'farmer') cmp = a.farmer.name.localeCompare(b.farmer.name);
    else if (sortKey === 'session') cmp = a.session.localeCompare(b.session);
    else cmp = Number(a.collectedMaunds) - Number(b.collectedMaunds);
    return sortDir === 'asc' ? cmp : -cmp;
  }), [entries, sortKey, sortDir]);

  const updateRow = (idx: number, field: keyof BulkRow, value: string | boolean) => {
    ...
    try {
      ...
      for (const row of toSave) {
        if (row.morning !== '' && Number(row.morning) >= 0) {
          promises.push(collectionApi.create({
            date, session: 'MORNING', farmerId: row.farmerId,
            collectedMaunds: Number(row.morning),
            qualityFatPercent: row.fatPercent ? Number(row.fatPercent) : undefined,
            qualityWaterAdded: row.waterAdded, qualityPassed: row.qualityPassed,
          }));
        }
        if (row.evening !== '' && Number(row.evening) >= 0) {
          promises.push(collectionApi.create({
            date, session: 'EVENING', farmerId: row.farmerId,
            collectedMaunds: Number(row.evening),
            qualityFatPercent: row.fatPercent ? Number(row.fatPercent) : undefined,
            qualityWaterAdded: row.waterAdded, qualityPassed: row.qualityPassed,
          }));
        }
      ...
      await load();
      setRows((prev) => prev.map((r) => ({ ...r, morning: '', evening: '', fatPercent: '', waterAdded: false, qualityPassed: true })));
  ...
  const historyHeaders: { key: SortKey; label: string }[] = [
    ...
    { key: 'collectedMaunds', label: `${t('maunds')} (${t('maundsUnit')})` },
  ];
    ...
    <div className="w-full space-y-4 animate-fadein">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{t('collectionHistory')}</p>
        </div>
        <input id="col-date" type="date" value={date} onChange={(e) => setDate(e.target.value)}
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="card p-4">
            <div className="text-xs text-gray-500 dark:text-slate-400 uppercase font-medium tracking-wide mb-1">{t('collectedStat')}</div>
      ...
      <div className="tab-group w-fit">
        <button type="button" onClick={() => setTab('input')}
          className={`tab-btn ${tab === 'input' ? 'active' : ''}`}>{t('inputTab')}</button>
        <button type="button" onClick={() => setTab('history')}
          className={`tab-btn ${tab === 'history' ? 'active' : ''}`}>{t('historyTab')}</button>
      </div>
      ...
      {tab === 'input' && (
        <div className="space-y-3">
            ...
            <div className="table-scroll">
              <table className="data-table min-w-[700px]">
                <thead>
                  <tr>
                    <th>{t('farmer')}</th>
                    <th className="text-gray-400 normal-case text-xs font-normal">{t('contractedLabel')}/{t('today').toLowerCase()}</th>
                ...
                <tbody>
                  {rows.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.farmerId} className={hasData ? 'bg-blue-50 dark:bg-blue-900/10' : ''}>
                        <td className="font-medium text-gray-900 dark:text-slate-100 whitespace-nowrap">{row.name}</td>
                        <td className="text-gray-400 text-xs">{row.contractedMaunds}</td>
                        <td>
                        ...
                        <td>
                          <input type="number" min="0" max="10" step="0.1" placeholder="e.g. 4.5"
                            value={row.fatPercent} disabled={isReadonly}
                            onChange={(e) => updateRow(idx, 'fatPercent', e.target.value)}
                            className="w-24 border border-gray-300 dark:border-slate-600 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 dark:bg-slate-800 dark:text-slate-100 disabled:opacity-50 disabled:cursor-not-allowed"
                          />
                        </td>
                        <td className="text-center">
                          <input type="checkbox" title={t('waterAdded')} checked={row.waterAdded} disabled={isReadonly}
                            onChange={(e) => updateRow(idx, 'waterAdded', e.target.checked)}
                            className="w-4 h-4 accent-red-500 disabled:cursor-not-allowed" />
                        </td>
                        <td className="text-center">
                          <input type="checkbox" title={t('quality')} checked={row.qualityPassed} disabled={isReadonly}
                            onChange={(e) => updateRow(idx, 'qualityPassed', e.target.checked)}
                            className="w-4 h-4 accent-green-500 disabled:cursor-not-allowed" />
                        </td>
                      </tr>
                    );
                  })}
                  ...
                  {!rows.length && (
                    <tr><td colSpan={7}><div className="empty-state"><span className="empty-state-icon">🌾</span><span className="empty-state-text">{t('noFarmers')}</span></div></td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={saveAll} disabled={saving || filledRows === 0 || isReadonly}
              className="btn btn-primary disabled:opacity-50 disabled:cursor-not-allowed">
              {saving ? t('saving') : t('saveEntries')}
            </button>
            {filledRows > 0 && (
              <button type="button"
                onClick={() => setRows((prev) => prev.map((r) => ({ ...r, morning: '', evening: '', fatPercent: '', waterAdded: false, qualityPassed: true })))}
          ...
          <div className="table-scroll">
            <table className="data-table">
              ...
              <tbody>
                {sortedEntries.map((e) => (
                  <tr key={e.id}>
                    <td className="font-medium text-gray-900 dark:text-slate-100">{e.farmer.name}</td>
                    <td>
                    ...
                    <td className="font-semibold text-gray-900 dark:text-slate-100" dir="ltr">
                      {Number(e.collectedMaunds).toFixed(2)} {t('maundsUnit')}
                    </td>
                    <td className="text-gray-600 dark:text-slate-400">
                      {e.qualityFatPercent != null ? `${e.qualityFatPercent}%` : '—'}
                    </td>
                    <td>
                      {e.qualityWaterAdded
                        ? <span className="badge badge-red">{t('yes') ?? 'Yes'}</span>
                        : <span className="badge badge-green">{t('no') ?? 'No'}</span>}
                    </td>
                    <td>
                      <span className={`badge ${e.qualityPassed ? 'badge-green' : 'badge-red'}`}>
                        {e.qualityPassed ? t('passed') : t('failed')}
                      </span>
                    </td>
                  </tr>
                ))}
                {!sortedEntries.length && (
                  <tr><td colSpan={6}><div className="empty-state"><span className="empty-state-icon">🥛</span><span className="empty-state-text">{t('noEntries')}</span></div></td></tr>
                )}
              </tbody>
            </table>
          </div>
